...
<h3 class="text-center multiEffect">요리 등록</h3>
	...
	<form action="recipeWrite?${_csrf.parameterName}=${_csrf.token}" method="post" enctype="multipart/form-data">
		...
		<div class="form-group">
			<label for="price">레시피가격</label>
			<input type="number" class="form-control" id="price" placeholder="레시피 가격" name="rPrice" required> 	
		</div>
		<div class="form-group">
			<label for="address">식당 주소</label>
			<input type="text" class="form-control" id="address" placeholder="식당 주소" name="rAddress" required> 	
		</div>
		<div class="form-group">
			<label for="content">레시피설명</label>
			<textarea class="form-control" id="content" placeholder="레시피 설명" name="rContent" rows="10" required></textarea>			
		</div>
		<button type="submit" class="btn btn-success">레시피등록</button>&nbsp;&nbsp; 
